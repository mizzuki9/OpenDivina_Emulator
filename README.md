...
Welcome to **OpenDivina_emulator**, an open-source reverse engineering and software preservation project dedicated to network protocol research and data interoperability for a 2010s classic MMORPG.
...
* **Independent Expression & Implementation:** All source code in this repository is 100% original, written from scratch. Technical understanding of proprietary file containers and cryptographic layers was derived through static assembly analysis of the legacy client binary under strict interoperability standards. No proprietary source code was copied or referenced.
...
* **Current Status:** Clean specification definitions and functional replication. **Zero leaked files are used or welcomed.**
...
1. **The Emulator is Blank:** This repository contains only the original server architecture. It has no game databases, text strings, or NPC dialogues.
...
We are currently analyzing the behavior of the network layer and formatting the packet structures:
* [x] **Network Architecture Design:** Setup the basic TCP server skeleton in C++.
* [ ] **Packet Analysis & Protocol Parsing:** Defining the transport layer protocol reverse engineering (debugging the Finite State Machine (FSM) of the connection).
...
### 🤝 Call for Collaboration (Interoperability Standards)
...
* **DO** share packet documentation, wirelogs (Wireshark `.pcap` files), or custom loader hooking logic.
...
歡迎來到 **OpenDivina_emulator**。本計畫是一個開源的逆向工程與數位軟體歷史保存專案，致力於對 2010 年代一款經典 MMORPG 的網絡傳輸協定與數據結構進行相容性與重組研究。

### ⚖️ 免責聲明 (至關重要)
* **絕不託管具版權之資產：** 本儲存庫（Repository）專案絕不託管、分發或洩漏任何屬於原開發商、營運商或版權持有人所有的原廠遊戲資產、伺服器執行檔、外流原始碼或客戶端檔案。
* **獨立原創表達與實作：** 本專案中的所有原始碼皆為 100% 獨立原創。本專案對專有數據容器（.biz, .pak）及加密層的技術理解，皆來自於對原廠舊版客戶端公開二進位檔案之靜態反組譯與結構分析（以滿足互通性研究之必要規格）。專案實作僅提取其公開的演算法邏輯與資料規格，未複製、重製或參考任何原廠具著作權之源碼錶達。
...
* **核心目標：** 針對舊版網絡封包結構與數據容器進行相容性與重組研究。
* **開發語言：** C++（用於服務端核心與引導模組）/ Python（用於工具與數據提取腳本）。
* **目前狀態：** 獨立的通訊規格定義與功能重組實作。**本專案不歡迎、亦絕不使用任何非法外流之原廠檔案。**
...
### 🛠️ 開發藍圖與目前進度
我們目前正專注於對網絡傳輸層進行分析，並為封包結構與數據格式定義相容的規格模型：
* [x] **網絡底層架構設計：** 使用 C++ 完成基本 TCP 服務端骨架搭建。
* [ ] **封包與通訊協定分析：** 正在解析並定義網絡握手階段之有限狀態機（Finite State Machine, FSM）通訊結構。
...
* **歡迎** 分享封包結構文件、網絡側錄檔案（Wireshark `.pcap` 檔）或原創的動態引導（Hooking）邏輯。
* 如果您對異步 C++ 網絡編程、Python 數據解析或動態鏈結庫攔截有深入研究，歡迎隨時提交 Issue 或 Pull Request 與我們共同完善此相容性架構！
